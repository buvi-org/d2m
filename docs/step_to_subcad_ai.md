...
Current accepted-pair and planner-capacity status is tracked in
`docs/zero_to_cad_status.md`. Planner coverage is only the attempt queue.
...
Current scan and guarded live pilot status are tracked in
`docs/zero_to_cad_status.md`.
The first AI-heavy accepted row is train global index 21963, generated by
`deepseek-chat` in one attempt and accepted by the original-STEP trusted policy.
The same row timed out with `deepseek-v4-pro`, so the current recommended
AI-heavy pilot model is `deepseek-chat`.
...
- Track the corpus-capacity gap explicitly in `docs/zero_to_cad_status.md`;
  current plannable rows cannot yield 100k accepted pairs unless unsupported
  families mature or more source STEP rows are added.

...
| Test | Result | Notes |
|------|--------|-------|
| `python test_agentic_translator.py` | PASS, 182/182 | Covers non-LLM translator components, strict Zero-to-CAD runner policy tests, AI-heavy unsupported-attempt selection, generated-code normalization/discipline checks, STEP-envelope prompt guards, sloped-trough/shell planner and prompt coverage, arbitrary-profile duct guidance, wedge workplane extrusion guidance, row-timeout failure handling, REPL integration, geometry comparison, feedback formatting, and mocked rich-comparison propagation. |
| `python test_subcad_tube_profile.py` | PASS, 8/8 | Covers axis-aligned circular and arbitrary-profile `tube_profile` geometry, union/replace behavior, and process-plan records. |
...
- `--scan-compatible` now uses the typed pure-operation planner. Current
  accepted-pair and planner-capacity status is tracked in
  `docs/zero_to_cad_status.md`; planner coverage is not verified translation
  success.
...
- Guarded live pilot status is tracked in `docs/zero_to_cad_status.md`.
...
- Preserve accepted-index manifests so verified rows count toward future
...
8. The STEP-to-SubCAD training set reaches 100,000 accepted original-STEP-verified Zero-to-CAD pairs, with current progress reported separately from planner coverage in `docs/zero_to_cad_status.md`.

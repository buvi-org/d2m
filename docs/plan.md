...
- Guarded live pilots currently record 19 accepted original-STEP-verified pairs.
...
- Preserve accepted-index manifests so the 19 verified rows count toward future
...
- Deterministic preflight now accepts layered retained row 31, top-retained
  ...
  top-chamfer box row 57, and hollow shell-cover row 58 against their original
  STEP
  targets; the next deterministic target should be chosen from the next guarded
  forward scan after row 58.
...
8. The STEP-to-SubCAD training set reaches 100,000 accepted original-STEP-verified Zero-to-CAD pairs, with current progress reported separately from planner coverage: 19 accepted pairs and 86,923 plannable rows out of 100,516 local rows as of 2026-05-21.

...
| Test | Result | Notes |
|------|--------|-------|
| `python test_agentic_translator.py` | PASS, 176/176 | Covers non-LLM translator components, strict Zero-to-CAD runner policy tests, AI-heavy unsupported-attempt selection, generated-code discipline checks, STEP-envelope prompt guards, sloped-trough prompt coverage, row-timeout failure handling, REPL integration, geometry comparison, feedback formatting, and mocked rich-comparison propagation. |

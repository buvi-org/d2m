...
generates executable pure SubCAD from STEP evidence. Current accepted-pair and
planner-capacity status is tracked only in
`docs/zero_to_cad_status.md`. Planner coverage is not success. A row counts only
after generated pure SubCAD executes, emits the manufacturing artifacts, and its
generated STEP matches the original Zero-to-CAD `model.step` under the trusted
comparison policy. Hybrid/imported opaque geometry, direct CadQuery
reconstruction, and skipped unsupported features are rejected for verified pairs.
The accepted index now re-checks saved comparison artifacts and requires strict
volume agreement (default maximum 5% error) in addition to the original-STEP
trusted geometry policy; older loose-volume matches are excluded until repaired.
...
Run `python -m src.data.build_zero_to_cad_accepted_index` to regenerate the local
deduplicated accepted index and summary from `runs/zero_to_cad_live_pilots`.
...
| Test | Status |
|------|--------|
| `python test_agentic_translator.py` | PASS, 182/182. Includes strict Zero-to-CAD runner policy tests, AI-heavy unsupported-attempt coverage, generated-code normalization/discipline checks, STEP-envelope prompt guards, sloped-trough/shell planner and prompt coverage, arbitrary-profile duct guidance, wedge workplane extrusion guidance, and row-timeout failure handling; live LLM runs are separate. |
| `python test_subcad_tube_profile.py` | PASS, 8/8. Covers axis-aligned circular and arbitrary-profile `tube_profile` geometry and process-plan records. |
| `python test_subcad_side_profile_cutout.py` | PASS, 6/6. Covers side-face triangular profile cutouts for XZ/YZ cross-sections. |
| `python test_subcad_sloped_floor.py` | PASS, 14/14. Covers `slope_cut`/`sloped_floor` wedge geometry, `shell_wall` sloped-shell geometry, process-plan records, toolpaths, and pass-plan metadata. |
